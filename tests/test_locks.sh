...
# ensure repo exists — use the actual repository_root from config
REPO_ROOT=$(python3 -c "from ddm.config import Config; c=Config('config/config.yaml'); print(c.repository_root)")
mkdir -p "$REPO_ROOT/raw" "$REPO_ROOT/ready"
python3 -c "from ddm.storage import Storage; Storage('$REPO_ROOT/ddm.db')" > /dev/null 2>&1 || true
...
# ---- helper: check output (strip ANSI escape codes before matching) ----
assert_contains() {  # $1=output $2=pattern $3=test_desc
    if echo "$1" | sed 's/\x1b\[[0-9;]*m//g' | grep -q "$2"; then pass "$3"; else fail "$3 (expected: $2)"; fi
}

# ---- clean state ----
cleanup() { rm -f "$REPO_ROOT/raw/.lock_"* "$REPO_ROOT/ready/.lock_release_"*; [ -n "$SLEEP_PID" ] && kill $SLEEP_PID 2>/dev/null || true; }
trap cleanup EXIT
cleanup
...
echo "$output"
echo ""
assert_contains "$output" "✓ Submit successful" "不同模块不阻塞"
...
# ══════════════════════════════════════════════════════════════════
mk_lock "$REPO_ROOT/raw" .lock_CPU_PV_ITER 99999 crashed_user 15

info "submit CPU (锁 15 分钟前, PID 99999 不存在 → 自动清除)"
output=$(python3 -m ddm submit -m CPU -t PV_ITER -s "test" 2>&1) || true
echo "$output"
echo ""
assert_contains "$output" "✓ Submit successful" "过期锁自动清除"
assert_contains "$output" "⚠.*过期锁.*自动清除" "显示过期锁警告"
...
# ══════════════════════════════════════════════════════════════════
mk_lock "$REPO_ROOT/raw" .lock_CPU_PV_ITER $$ live_user 15

info "submit CPU (锁 PID=$$ 活着, 虽然 15 分钟前 → 自动清除但有警告)"
output=$(python3 -m ddm submit -m CPU -t PV_ITER -s "test" 2>&1) || true
echo "$output"
echo ""
assert_contains "$output" "✓ Submit successful" "活 PID 也会清除"
assert_contains "$output" "仍在运行.*Ctrl.C" "显示运行警告"
...
echo "$output"
echo ""
assert_contains "$output" "lock exists" "新鲜锁拒绝"
...
# ══════════════════════════════════════════════════════════════════
mk_lock "$REPO_ROOT/ready" .lock_release_PV_ITER $$ release_user 0

info "submit CPU @ PV_ITER (PV_ITER 正在 release)"
output=$(python3 -m ddm submit -m CPU -t PV_ITER -s "test" 2>&1) || true
echo "$output"
echo ""
assert_contains "$output" "Release.*提交被阻断" "release 锁阻塞 submit"
...
echo "$output"
echo ""
assert_contains "$output" "✓ Submit successful" "不同 tag submit 不阻塞"
...
echo "$output"
echo ""
assert_contains "$output" "Release.*发布被阻断" "release 锁阻塞 release"
...
echo "$output"
echo ""
assert_contains "$output" "✓ Release successful" "不同 tag release 不阻塞"
...
# ══════════════════════════════════════════════════════════════════
mk_lock "$REPO_ROOT/raw" .lock_DDR_PV_ITER 99999 crashed_user 20
mk_lock "$REPO_ROOT/raw" .lock_CPU_BASE_CLEAN $$ live_user 20

info "ddm check 应分别报告过期锁和运行中的锁"
output=$(python3 -m ddm check 2>&1) || true
echo "$output"
echo ""
assert_contains "$output" "进程已退出" "check 检测过期锁"
assert_contains "$output" "长期运行中的锁\|仍在运行" "check 检测运行锁"
...
# ══════════════════════════════════════════════════════════════════
mk_lock "$REPO_ROOT/raw" .lock_CPU_PV_ITER 99999 wangshuai 15

info "lisi 提交 CPU (wangshuai 已崩溃, 锁 15 分钟前 → 自动清除)"
output=$(python3 -m ddm submit -m CPU -t PV_ITER -s "lisi takeover" 2>&1) || true
echo "$output"
echo ""
assert_contains "$output" "✓ Submit successful" "崩溃后自动清除"
assert_contains "$output" "crashed_user\|wangshuai" "显示原用户"
...
mk_lock "$REPO_ROOT/raw" .lock_CPU_PV_ITER $$ submit_user 1

info "release -A PV_ITER (CPU 正在 submit → 阻断)"
output=$(python3 -m ddm release -t PV_ITER -A -v V100 2>&1) || true
echo "$output"
echo ""
assert_contains "$output" "CPU.*submit.*完成" "release 被 submit 阻塞"
